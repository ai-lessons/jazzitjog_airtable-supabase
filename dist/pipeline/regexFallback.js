...
exports.parseHeights = parseHeights;
exports.parseWeight = parseWeight;
exports.parsePriceToUSD = parsePriceToUSD;
const FX = {
    USD: 1,
    EUR: 1.08,
    GBP: 1.26,
...
function isValidPriceUSD(p) { return Number.isFinite(p) && p >= 40 && p <= 500; }
function toNum(s) {
    const x = Number(s.replace(/[^\d.\-]/g, ""));
    return Number.isFinite(x) ? x : null;
}
function gramsFromOz(oz) { return Math.round(oz * 28.3495); }
function near(text, idx, radius = 24) {
    return text.slice(Math.max(0, idx - radius), Math.min(text.length, idx + radius)).toLowerCase();
}
function looksLikePlatform(ctx) {
    return /\bplatform\b/.test(ctx) || /\bstacked\b/.test(ctx);
}
// --- Heights (stack) ---
function parseHeights(textRaw) {
    ...
    for (const re of pairs) {
        ...
        while ((m = re.exec(text))) {
            ...
            if (h && f && h <= 60 && f <= 60) {
                out.heel ??= h;
                out.forefoot ??= f;
                out.drop ??= Math.round((h - f) * 100) / 100;
                return out;
            }
        }
    }
...
// --- Price (→ USD) ---
function parsePriceToUSD(textRaw) {
    const text = textRaw.replace(/\s+/g, " ");
...
function convertToUSD(val, cur) {
    const rate = FX[cur.toUpperCase()];
    return rate ? val * rate : null;
}
function round2(n) { return Math.round(n * 100) / 100; }
//# sourceMappingURL=regexFallback.js.map