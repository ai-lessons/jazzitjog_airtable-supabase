...
exports.isRunningShoeArticle = isRunningShoeArticle;
exports.analyzeTitleForContext = analyzeTitleForContext;
exports.matchesTitleAnalysis = matchesTitleAnalysis;
const logger_1 = require("../../core/logger");
const mapping_1 = require("../../core/mapping");
/**
 * Check if text contains running shoe keywords (multilingual)
 */
function hasRunningShoeKeywords(text) {
    const shoeKeywords = [
        // English
        /\bshoe/i,
        /\bsneaker/i,
 ...
 */
function hasReviewKeywords(text) {
 ...
 */
function hasNonShoeKeywords(text) {
 ...
 */
function hasShoeCharacteristics(content) {
 ...
 */
function isRunningShoeArticle(title, content) {
    const titleLower = title.toLowerCase().trim();
    // ========== STAGE 1: Title Analysis ==========
    // 1.1: Check for NON-shoe keywords in title → REJECT immediately
    if (hasNonShoeKeywords(titleLower)) {
        logger_1.logger.info('Article rejected: non-shoe keywords in title', { title });
        return false;
    }
    // 1.2: Check for SHOE keywords in title → ACCEPT immediately
    if (hasRunningShoeKeywords(titleLower)) {
        logger_1.logger.info('Article accepted: shoe keywords in title', { title });
        return true;
    }
    // 1.3: Title is unclear (no shoe keywords, no non-shoe keywords)
    logger_1.logger.debug('Title unclear, analyzing content...', { title });
    // ========== STAGE 2: Content Analysis ==========
    if (!content) {
        logger_1.logger.info('Article rejected: unclear title, no content to analyze', { title });
        return false;
    }
    const contentLower = content.toLowerCase();
    ...
    if (hasShoeKeywords || hasCharacteristics) {
        logger_1.logger.info('Article accepted: shoe indicators in content', {
            title,
            hasShoeKeywords,
            hasCharacteristics
        });
    ...
    if (hasNonShoeKeywords(contentLower)) {
        logger_1.logger.info('Article rejected: non-shoe keywords in content', { title });
        return false;
    }
    // 2.3: Unclear content (no positive signals, no negative signals) → REJECT by default
    logger_1.logger.info('Article rejected: no clear indicators in content', { title });
    return false;
}
/**
 ...
 */
function analyzeTitleForContext(title) {
    const normalized = title.toLowerCase().trim();
    // Try to detect specific model mention
    const specificMatch = detectSpecificModel(normalized);
    if (specificMatch) {
        logger_1.logger.debug('Title analysis: specific model detected', specificMatch);
        return {
            scenario: 'specific',
    ...
    if (brandMatch) {
        logger_1.logger.debug('Title analysis: brand-only detected', brandMatch);
        return {
            scenario: 'brand-only',
    ...
    logger_1.logger.debug('Title analysis: general article', { title });
    return {
        scenario: 'general',
        confidence: 0.8,
    };
 ...
 */
function detectSpecificModel(title) {
    // Check against known models
    for (const [model, brand] of Object.entries(mapping_1.MODEL_TO_BRAND)) {
        ...
        if (match) {
            ...
            if (match.length === 4) {
                ...
                if (brandsList.includes(potentialBrand)) {
                    ...
                    brand = mapping_1.MODEL_TO_BRAND[modelKey] || null;
                }
            }
            if (brand && model) {
                return {
 ...
 */
function detectBrandOnly(title) {
    ...
    for (const brand of brands) {
        if (title.includes(brand)) {
            ...
            const hasModelName = Object.keys(mapping_1.MODEL_TO_BRAND).some(m => title.includes(m.toLowerCase()));
...
function normalizeBrand(brand) {
    ...
    if (mapping_1.BRAND_ALIASES[upperBrand]) {
        return mapping_1.BRAND_ALIASES[upperBrand];
    }
    // Otherwise use core normalization
    return (0, mapping_1.normalizeBrand)(brand);
}
/**
 * Check if sneaker matches title analysis
 */
function matchesTitleAnalysis(sneaker, analysis) {
...
//# sourceMappingURL=title_analysis.js.map