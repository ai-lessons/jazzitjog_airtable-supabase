...
exports.SimpleSneakerParser = void 0;
const openai_1 = __importDefault(require("openai"));
class SimpleSneakerParser {
    openai;
    constructor(apiKey) {
    ...
    async parseArticle(data) {
        ...
        const sneakers = await this.extractSneakers(data.content, data.title, titleAnalysis, data.date, data.source_link);
        return {
            ...data,
            sneakers,
        };
    ...
    async extractSneakers(content, title, titleAnalysis, date, source) {
    ...
    filterRelevantSneakers(sneakers, titleAnalysis, title, content) {
        return sneakers.filter(sneaker => {
            return this.isRelevantSneaker(sneaker, titleAnalysis, title, content);
        });
    }
    isRelevantSneaker(sneaker, titleAnalysis, title, content) {
    ...
    countBrandMentions(brand, content) {
        const lowerContent = content.toLowerCase();
        const lowerBrand = brand.toLowerCase();
        const matches = lowerContent.match(new RegExp(lowerBrand, 'g'));
        return matches ? matches.length : 0;
    }
    isModelSubstantiallyDiscussed(model, content) {
        const lowerContent = content.toLowerCase();
    ...
    isModelMentioned(model, content) {
        const lowerContent = content.toLowerCase();
        const lowerModel = model.toLowerCase();
        // Split model into key words and check if most are present
        const modelWords = lowerModel.split(/\s+/).filter(word => word.length > 2 && !/^(the|and|or|in|on|at|for|with)$/.test(word));
        if (modelWords.length === 0)
            return false;
...
- Model names must be complete (e.g., "Evo SL" not "Evo")`;
    ...
    buildExtractionPrompt(content, title, titleAnalysis) {
        let instruction = "";
        if (titleAnalysis.scenario === 'specific') {
...
Content: ${content}`;
    ...
    normalizeSneakers(sneakers, date, source) {
        return sneakers
            .filter((s) => this.isValidExtraction(s))
            .map((s) => ({
    ...
    normalizePlate(plate) {
        if (typeof plate === "boolean")
            return plate;
        if (typeof plate === "string") {
            const s = plate.toLowerCase();
            if (s.includes("carbon") || s.includes("plate"))
                return true;
            if (s.includes("no") || s.includes("none"))
                return false;
        }
        return undefined;
    }
    normalizeWaterproof(waterproof) {
        if (typeof waterproof === "boolean")
            return waterproof;
        if (typeof waterproof === "string") {
            const s = waterproof.toLowerCase();
            if (s.includes("gore-tex") || s.includes("waterproof") || s.includes("water-resistant"))
                return true;
            if (s.includes("not") || s.includes("no"))
                return false;
        }
        return undefined;
    }
    ...
    normalizeDrop(drop, heel, forefoot) {
        let d = Number(drop);
    ...
    normalizeCushioning(cushioning) {
        if (!cushioning)
            return undefined;
        const s = String(cushioning).toLowerCase();
        if (s.includes("firm") || s.includes("stiff"))
            return "firm";
        if (s.includes("max") || s.includes("plush") || s.includes("high"))
            return "max";
        if (s.includes("balanced") || s.includes("moderate") || s.includes("medium"))
            return "balanced";
        return undefined;
    }
    normalizeWidth(width) {
        ...
        if (s.includes("narrow") || s.includes("slim"))
            return "narrow";
        if (s.includes("wide") || s.includes("2e") || s.includes("4e"))
            return "wide";
        if (s.includes("standard") || s.includes("regular") || s.includes("medium"))
            return "standard";
        return undefined;
    }
    normalizeBreathability(breathability) {
        if (!breathability)
            return undefined;
...
exports.SimpleSneakerParser = SimpleSneakerParser;
