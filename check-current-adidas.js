...
async function checkCurrentAdidas() {
  ...
  console.log('🔍 Checking current Adidas records in database...\n');

  // Get all Adidas records
  const { data: adidasRecords, error } = await supabase
    .from('shoe_results')
    .select('article_id, brand_name, model, record_id')
  ...
  console.log(`Found ${adidasRecords.length} Adidas records in current database:`);
  ...
  for (const articleId of articleIds) {
    ...
    if (airtableRecord) {
      ...
      console.log(`❌ Article ${articleId} not found in Airtable`);
    }
  }
  ...
  if (irrelevantCount > 0) {
    console.log('\n🚨 FILTERING STILL NOT WORKING! System extracting Adidas from non-Adidas articles!');
  } else {
    console.log('\n🎉 SUCCESS: All Adidas extractions are from relevant articles!');
  }
}

checkCurrentAdidas().catch(console.error);