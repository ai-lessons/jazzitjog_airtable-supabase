...
function scoreSnippet(snippet: string): number {
  ...
  // Drop pattern (mm)
  if (/(\d+)\s*mm.*drop|drop.*(\d+)\s*mm/i.test(snippet)) score += 2;
...
function buildBestCluster(topSnippets: string[]): {
  ...
  for (let i = 0; i < topSnippets.length; i++) {
    ...
    // Extract drop
    const dropMatch = snippet.match(/(\d{1,2})\s*mm.*drop|drop.*(\d{1,2})\s*mm/i);
    if (dropMatch) {
      const numMatch = dropMatch[0].match(/\d{1,2}/);
      if (numMatch) {
        const drop = parseInt(numMatch[0], 10);
        if (drop >= 0 && drop <= 25) {
...
function extractSpecsFromWindows(
  ...
  for (const window of windows) {
    ...
    if (priceMatches) {
      for (const match of priceMatches) {
        ...
        if (value >= 50 && value <= 500) {
          ...
          rawStrings.price.push(match);
        }
      }
    }
    
    // Extract drop (mm)
    const dropMatches = text.match(/(\d{1,2})\s*mm.*drop|drop.*(\d{1,2})\s*mm/gi);
    if (dropMatches) {
      for (const match of dropMatches) {
        ...
        if (numMatch) {
          ...
          if (value >= 0 && value <= 25) {
            ...
            rawStrings.drop.push(match);
          }
        }
      }
    }
    
    // Extract weight (grams or ounces)
    const gramMatches = text.match(/(\d{2,4})\s*(?:g|grams)\b/gi);
    if (gramMatches) {
      for (const match of gramMatches) {
        ...
        if (numMatch) {
          ...
          if (value >= 50 && value <= 1000) {
            ...
            rawStrings.weight.push(match);
          }
        }
      }
    }
    
    const ozMatches = text.match(/(\d{1,2}(?:\.\d)?)\s*(?:oz|ounces)\b/gi);
    if (ozMatches) {
      for (const match of ozMatches) {
        const numMatch = match.match(/\d+(?:\.\d+)?/);
        if (numMatch) {
          const oz = parseFloat(numMatch[0]);
          if (oz >= 3 && oz <= 30) {
    ...
    // Extract heel (mm)
    const heelMatches = text.match(/(\d{2})\s*mm.*heel|heel.*(\d{2})\s*mm/gi);
    if (heelMatches) {
      for (const match of heelMatches) {
        const numMatch = match.match(/\d{2}/);
        if (numMatch) {
          const value = parseInt(numMatch[0]);
          if (value >= 5 && value <= 60) {
            allValues.heel_mm.add(value);
            rawStrings.heel.push(match);
          }
        }
      }
    }
    ...
    // Extract forefoot (mm)
    const forefootMatches = text.match(/(\d{2})\s*mm.*forefoot|forefoot.*(\d{2})\s*mm/gi);
    if (forefootMatches) {
      for (const match of forefootMatches) {
        const numMatch = match.match(/\d{2}/);
        if (numMatch) {
          const value = parseInt(numMatch[0]);
          if (value >= 5 && value <= 60) {
            allValues.forefoot_mm.add(value);
            rawStrings.forefoot.push(match);
          }
        }
      }
    }
  ...
  // Convert sets to arrays for analysis
  ...
  const heelValues = Array.from(allValues.heel_mm);
    ...
    const workerCode = `
      ...
      function extractMultiModelFromTables($, debugMode) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const $row = $(row);
          const cells = $row.find('th, td');
          if (cells.length !== modelNames.length + 1) continue;
  ...
  for (const row of data || []) {
    ...
    try {
      ...
      if (usingContent) {
        ...
        // Step 12: Before script end for this row
        stage = 'done';
        emitStage(stage);
        logger.info({
          ID: row.ID,
