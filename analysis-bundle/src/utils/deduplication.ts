 ...
 */
export function generateModelKey(brandName: string, model: string): string {
 ...
 */
export function mergeShoeResults(existing: ShoeResult, incoming: ShoeResult): ShoeResult {
  const merged: ShoeResult = { ...existing };

  // Helper to determine if a value is "richer" than another
  const isRicher = (newVal: any, oldVal: any): boolean => {
 ...
 */
export function isPayloadRicher(candidate: ShoeResult, existing: ShoeResult): boolean {
  let candidateScore = 0;
  let existingScore = 0;

  // Count non-null fields and apply weights
  Object.keys(candidate).forEach(key => {
    const candidateVal = (candidate as any)[key];
 ...
 */
export function deduplicateInDocument(models: ShoeResult[]): ShoeResult[] {
  const modelMap = new Map<string, ShoeResult>();
