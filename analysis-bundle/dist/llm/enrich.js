...
class SneakerEnricher {
    ...
    async enrichArticle(articleId, title, content, articleUrl) {
        try {
            ...
            const sourceId = (0, deduplication_1.generateSourceId)(String(articleId), articleUrl, content);
            const shoeResults = validatedData.map(data => ({
            ...
            const deduplicatedResults = (0, deduplication_1.deduplicateInDocument)(shoeResults);
    ...
    validateAndCleanData(rawData, articleUrl) {
        ...
        for (const item of rawData) {
            try {
                ...
                if (!(0, sneakerValidation_1.shouldIncludeInSneakerDB)(cleanedItem.brand, cleanedItem.model)) {
