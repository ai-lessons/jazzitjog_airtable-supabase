...
const MULTIWORD_BRANDS_HEAD: Array<[string, string, string]> = [
  ['Li', 'Ning', 'Li-Ning'],
...
function cleanTitle(raw: string): string {
  ...
  if (s && s === s.toUpperCase()) s = toTitleCase(s);
  return s;
}
...
function stripTrailingReviewWords(s: string): string {
...
function refineModel(model: string): string {
...
function startsWithWhitelistedBrand(s: string): boolean {
  return HEAD_WHITELIST.some(rx => rx.test(s));
}

function stripInitialNoise(s: string): string {
  let t = s.trim();
  let guard = 0;
  while (guard++ < 5) {
    const before = t;
...
function findKnownBrandAnywhere(s: string): { brand: string; after: string } | null {
...
function extractModelFromAfter(afterRaw: string): string | null {
  ...
  const isModelish = (t: string) => /^[A-Za-z0-9][A-Za-z0-9\-\/]*$/.test(t);
  for (const tok of tokens) {
    if (AFTER_NOISE.test(tok)) break;
    if (!isModelish(tok)) break;
...
function canonicalizeModel(brand: string, model: string): string {
...
function splitBrandModelFromHead(headRaw: string): { brand: string | null; model: string | null } {
  ...
  if (tokens[0].toLowerCase() === 'mount' && tokens[1]?.toLowerCase() === 'to' && tokens[2]?.toLowerCase() === 'coast') {
    ...
    if (!model) return { brand: null, model: null };
 ...
 */
export function brandModelFromTitle(rawTitle: string): {
  brand: string | null; model: string | null; confidence: number;
} {
  let t = cleanTitle(rawTitle);
  if (!t) return { brand: null, model: null, confidence: 0 };
