...
async function testFullPipeline() {
  ...
  const directResults = await fromAirtableToShoeInputs(mockAirtableRecords);
  ...
  const mappedResults = await mapFromAirtable(mockAirtableRecords);
  ...
  const validationResults = directResults.map((shoe, i) => {
    ...
    // Обязательные поля
    if (!shoe.article_id) errors.push('Missing article_id');
    if (!shoe.record_id) errors.push('Missing record_id');
    if (!shoe.brand_name) errors.push('Missing brand_name');
    if (!shoe.model) errors.push('Missing model');
    if (!shoe.model_key) errors.push('Missing model_key');
  ...
  const articleGroups = new Map<number, typeof directResults>();
