...
export function convertToUSD(amount: number, code: string): number | null {
  switch (code) {
...
export function detectCurrency(text: string): string | null {
  ...
  if (/\$\s*\d/.test(t) || /\bUSD\b/.test(t)) return "USD";
  if (/€\s*\d/.test(t) || /\bEUR\b/.test(t)) return "EUR";
  if (/£\s*\d/.test(t) || /\bGBP\b/.test(t)) return "GBP";
  if (/\bJPY\b/.test(t) || /¥\s*\d/.test(t)) return "JPY";
  if (/\bCAD\b/.test(t)) return "CAD";
  if (/\bAUD\b/.test(t)) return "AUD";
  if (/\bCHF\b/.test(t)) return "CHF";
  if (/\bSEK\b/.test(t)) return "SEK";
  if (/\bNOK\b/.test(t)) return "NOK";
  if (/\bDKK\b/.test(t)) return "DKK";
  if (/\bPLN\b/.test(t)) return "PLN";
  return null;
}
