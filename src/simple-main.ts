...
import { SimpleSyncProcessor } from './simple-sync';
...
async function main() {
  ...
  if (!config.supabase.key) missing.push('SUPABASE_SERVICE_ROLE_KEY');
...
async function handleSync(processor: SimpleSyncProcessor, args: string[]) {
...
async function handleStats(processor: SimpleSyncProcessor) {
...
async function handleTest(processor: SimpleSyncProcessor, args: string[]) {
...
async function handleClearDb(processor: SimpleSyncProcessor) {
