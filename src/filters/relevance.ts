...
export type Irrelevance =
  | { ok: true }
  | { ok: false; reason: 'listicle' | 'apparel' | 'nonshoe' | 'badbrand' | 'badmodel' };

export function detectIrrelevant(title: string, modelKey: string, model: string, brand?: string): Irrelevance {
  const hay = `${title || ''} ${modelKey || ''} ${model || ''}`;

  if (LISTICLE.test(hay)) return { ok: false, reason: 'listicle' };
  if (APPAREL.test(hay))  return { ok: false, reason: 'apparel' };
  if (NONSHOE.test(hay))  return { ok: false, reason: 'nonshoe' };
  if (brand && BAD_BRAND.test(String(brand).trim())) return { ok: false, reason: 'badbrand' };
  if (model && BAD_MODEL.test(String(model).trim())) return { ok: false, reason: 'badmodel' };

  // Проверка длины - отсеиваем слишком длинные названия (характерны для заголовков статей)
  if (brand && String(brand).trim().length > 25) return { ok: false, reason: 'badbrand' };
  if (model && String(model).trim().length > 50) return { ok: false, reason: 'badmodel' };

  // Проверка на слишком много слов (больше 6 слов в модели = подозрительно)
  if (model && String(model).trim().split(/\s+/).length > 6) return { ok: false, reason: 'badmodel' };
