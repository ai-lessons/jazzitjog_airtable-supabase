import { extractMultipleModels, detectModelHeadings, collectModelBlock, extractSpecs } from './extractMultiModel';
...
function runTests() {
  ...
  const headings = detectModelHeadings(sampleContent);
  ...
  const models = extractMultipleModels(sampleContent);
