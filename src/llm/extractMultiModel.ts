 ...
 */
function extractUseAndSurface(heading: string, description: string): { primary_use: string | null; surface_type: string | null } {
  const combined = `${heading} ${description}`.toLowerCase();

  let primary_use: string | null = null;
  let surface_type: string | null = null;
 ...
 */
export function extractMultipleModels(content: string): ExtractedSpecs[] {
