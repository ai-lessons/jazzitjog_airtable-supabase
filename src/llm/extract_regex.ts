...
import { refineModelName } from "../transform/fields";
...
// Use SneakerSpec from types (backward compatibility alias)
export type ExtractedSpecs = SneakerSpec;

/**
 * Detect model headings in structured content
 */
export function detectModelHeadings(content: string): HeadingMatch[] {
 ...
 */
export function collectModelBlock(content: string, heading: HeadingMatch, nextHeading?: HeadingMatch): string {
 ...
 */
export function extractSpecs(block: string, heading: HeadingMatch): Partial<ExtractedSpecs> {
 ...
 */
function parseBrandModel(brandModel: string): { brand_name: string | null; model: string | null } {
  const trimmed = brandModel.trim();
  if (!trimmed) return { brand_name: null, model: null };
  ...
  const brand = parts[0];
  const model = refineModelName(brand, parts.slice(1).join(' '));
 ...
 */
function extractHeights(text: string): { heel_height: number | null; forefoot_height: number | null } {
  let heel_height: number | null = null;
  let forefoot_height: number | null = null;
 ...
 * Extract drop value
 */
function extractDrop(text: string): number | null {
 ...
 */
function extractUseAndSurface(heading: string, description: string): { primary_use: string | null; surface_type: "road" | "trail" | null } {
  const combined = `${heading} ${description}`.toLowerCase();

  let primary_use: string | null = null;
  let surface_type: "road" | "trail" | null = null;
 ...
 */
function extractWaterproof(text: string): boolean | null {
 ...
 */
function extractCarbonPlate(text: string): boolean | null {
...
function normalizeCushion(x: any): "firm"|"balanced"|"max"|null {
...
function normalizeSurface(x: any): "road"|"trail"|null {
  const s = String(x ?? "").toLowerCase();
...
function normalizeWidth(x: any): "narrow"|"standard"|"wide"|null {
  const s = String(x ?? "").toLowerCase();
 ...
 */
export function normalizeSpecs(raw: Partial<ExtractedSpecs>): ExtractedSpecs {
  return {
 ...
 */
function detectUnstructuredModels(content: string): HeadingMatch[] {
  const models: HeadingMatch[] = [];
 ...
 */
function isValidModelPart(modelPart: string): boolean {
  if (!modelPart || modelPart.length < 2) return false;
 ...
 */
export function extractWithRegex(content: string): SneakerSpec[] {
  const metrics = getMetrics();
  logger.debug('Starting regex extraction', { contentLength: content.length });

  const results = extractMultipleModels(content);

  logger.info('Regex extraction completed', { count: results.length });
  if (results.length > 0) {
 ...
 */
function extractMultipleModels(content: string): ExtractedSpecs[] {
  let allModels: HeadingMatch[] = [];
  ...
  const models: ExtractedSpecs[] = [];
  const dedupeMap = new Map<string, ExtractedSpecs>();
  ...
  for (let i = 0; i < allModels.length; i++) {
    ...
    if (!hasRunningContext) {
      logger.debug(`Skipping ${normalizedSpecs.brand_name} ${normalizedSpecs.model} - missing Use or Surface (likely not a shoe)`);
      continue;
    }
 ...
 * Additional validation for model quality
 */
function isHighQualityModel(brand: string, model: string): boolean {
 ...
 */
function areSimilarModels(model1: ExtractedSpecs, model2: ExtractedSpecs): boolean {
  // Must be same brand
 ...
 */
function mergeModels(existing: ExtractedSpecs, newModel: ExtractedSpecs): ExtractedSpecs {
